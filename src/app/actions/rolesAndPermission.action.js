...
export const RolesAndPermissionAction = {
  ...
  refreshRoles: () => ({type: RolesAndPermissionMap.REFRESH_ROLES}),
};
...
export const addRoleAsync = (roleName) => async (dispatch, getState) => {
  try {
    ...
    if (data.response && data.response.responseCode === 200) {
      ...
      dispatch(showSuccessSnackbar('success', "Role Added Successfully", 300000));
    } else {
...
export const editRoleAsync = (roleName, roleId) => async (dispatch, getState) => {
  try {
    ...
    if (data.response && data.response.responseCode === 200) {
      ...
      return dispatch(showSuccessSnackbar('success', "Role Updating Successfully", 3000));
    }
