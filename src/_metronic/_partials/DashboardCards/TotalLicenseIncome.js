...
import { getTotalLicenseIncomeAsync } from '../../../app/actions/dashboard.action';
import moment from 'moment';

const TotalLicenseIncome = () => {

    const [date, setDate] = useState("");

    const dispatch = useDispatch();

    const { totalLicenseIncome } = useSelector(state => state.dashboard)

    useEffect(() => {
        dispatch(getTotalLicenseIncomeAsync(date))
    }, [date])
                ...
                <div className="row no-gutters align-items-center">
                    <div className="col mr-2">
                        ...
                        <div className="row_data">
                            <div className="h4 mb-0 font-weight-bold text-gray-800">${parseFloat(totalLicenseIncome).toFixed(2)}</div>
                            <Form.Control
                                as="select"
                                defaultValue={date}
...
export default TotalLicenseIncome;
